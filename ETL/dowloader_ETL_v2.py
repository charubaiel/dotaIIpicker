...
from dagster import MetadataValue,Output,RunRequest,DefaultSensorStatus,DefaultScheduleStatus,DagsterEventType,EventRecordsFilter
import pandas as pd 
...
def optimize_data(context,prepare_data:pd.DataFrame)->pd.DataFrame:
    ...
    match_table = pd.json_normalize(prepare_data['players'].explode('players')).dropna(axis=1).apply(pd.to_numeric,errors='coerce',downcast='unsigned').dropna(axis=1)
...
@asset(description='update raw db data',
        config_schema={"db_path": str},
        group_name='save')
def update_raw(context,prepare_data:pd.DataFrame)->None:
    result_exploded_data = pd.json_normalize(prepare_data['players'].explode('players')).dropna(axis=1).apply(pd.to_numeric,errors='coerce',downcast='unsigned').dropna(axis=1)
...
@asset(description='update h5 result matrix',
        ...
        group_name='save')
def update_optimized_base(context,optimize_data)->None:
    ...
    with sqlite3.connect(context.op_config['db_path']) as connect:
        ...
        total_data.to_hdf(context.op_config['file_path'],key='matrix_table',mode='w',complevel=9,index=False)
